...
class Ex05_ArrayColumnType: R2dbcExposedTestBase() {
    ...
    fun `create and drop array columns`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `create missing columns with array defaults`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `array max size`(testDB: TestDB) = runTest {
        val maxArraySize = 5
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `select using array get`(testDB: TestDB) = runTest {
        withArrayTestTable(testDB) {
            val numInput = listOf(1, 2, 3)
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `select using array slice`(testDB: TestDB) = runTest {
        withArrayTestTable(testDB) {
            val numInput = listOf(1, 2, 3)
            ArrayTestTable.insert {
                it[numbers] = numInput
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `array literal and array param`(testDB: TestDB) = runTest {
        withArrayTestTable(testDB) {
            /**
             * ```sql
             * -- Postgres
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `array column update`(testDB: TestDB) = runTest {
        withArrayTestTable(testDB) {
            val id1 = ArrayTestTable.insertAndGetId {
                it[doubles] = null
            }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `array column upsert`(testDB: TestDB) = runTest {
        withArrayTestTable(testDB) {
...
//    }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `array column with all any ops`(testDB: TestDB) = runTest {
        withArrayTestTable(testDB) {
            /**
             * ```sql
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `insert array of byte arrays`(testDB: TestDB) = runTest {
    ...
    private suspend fun withArrayTestTable(
        testDB: TestDB,
