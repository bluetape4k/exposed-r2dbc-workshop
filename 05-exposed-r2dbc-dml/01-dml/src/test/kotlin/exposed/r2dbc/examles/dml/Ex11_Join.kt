...
class Ex11_Join: R2dbcExposedTestBase() {
    ...
    fun `manual join`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `triple join`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `many-to-many join`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `cross join`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `multiple reference join 01`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `multiple reference join 02`(testDB: TestDB) = runTest {
        val foo = object: IntIdTable("foo") {
            val baz = integer("baz").uniqueIndex()
        }
        val bar = object: IntIdTable("bar") {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `join with alias 01`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `join with join`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, users, userData ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `join with additional constraint`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `no warnings on left join regression`(testDB: TestDB) = runTest {
