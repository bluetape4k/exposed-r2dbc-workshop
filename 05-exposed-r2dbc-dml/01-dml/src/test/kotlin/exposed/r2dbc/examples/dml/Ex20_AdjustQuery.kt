...
class Ex20_AdjustQuery: R2dbcExposedTestBase() {
    ...
    fun `adjust query slice`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `adjust query slice with empty list will throw`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `adjust query column set`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `adjust query where`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, users, _ ->
            val queryAdjusted: Query = (users innerJoin cities)
                .select(users.name, cities.name)
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `query or where`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, users, _ ->
