...
class Ex17_Union: R2dbcExposedTestBase() {
    ...
    fun `union within limit`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `union with limit and offset`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `count of union query`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, users, userData ->
            val andreyQuery = users.selectAll().where { users.id eq "andrey" }
            val sergeyQuery = users.selectAll().where { users.id eq "sergey" }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `union within orderBy`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `union of two queries`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
            val andreyQuery = users.selectAll().where { users.id eq "andrey" }
            val sergeyQuery = users.selectAll().where { users.id eq "sergey" }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `intersect with three queries`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `except with two query`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB !in TestDB.ALL_MYSQL }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `except of three queries`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB !in TestDB.ALL_MYSQL }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `except of two excepts queries`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB !in TestDB.ALL_MYSQL }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `union of more than two query`(testDB: TestDB) = runTest {
        // Assumptions.assumeTrue { testDB !in TestDB.ALL_MYSQL }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `union of sorted queries`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `union of limited queries`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
            val andreyOrSergeyQuery = users.selectAll()
                .where { users.id inList setOf("andrey", "sergey") }
                .limit(1)
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `union of sorted and limited queries`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `union with distinct results`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
            val andreyQuery = users.selectAll().where { users.id eq "andrey" }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `union with all results`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
            val andreyQuery = users.selectAll().where { users.id eq "andrey" }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `union with all results of three queries`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
            val andreyQuery = users.selectAll().where { users.id eq "andrey" }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `union with expressions`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `union with expression and alias`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
            val exp1a = intLiteral(10)
            val exp1b = intLiteral(100)
            val exp2a = stringLiteral("aaa")
