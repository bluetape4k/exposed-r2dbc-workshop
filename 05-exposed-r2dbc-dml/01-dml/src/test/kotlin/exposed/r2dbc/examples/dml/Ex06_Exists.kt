 ...
 */
class Ex06_Exists: AbstractR2dbcExposedTest() {
    ...
    fun `exists example 01`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `exists in a slice`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, userData ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `exists examples 02`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, userData ->
            val rows = users
                .selectAll()
                .where {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `exists examples 03`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, userData ->
            val rows = users
                .selectAll()
                .where {
