...
class Ex04_Upsert: R2dbcExposedTestBase() {
    ...
    fun `upsert with PK conflict`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `upsert with composite PK conflict`(testDB: TestDB) = runTest {
        /**
         * ```sql
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `upsert with all columns in PK`(testDB: TestDB) = runTest {
        /**
         * ```sql
         * CREATE TABLE IF NOT EXISTS tester (
         *      user_id VARCHAR(32),
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `upsert with unique index conflict`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `upsert with manual conflict keys`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `upsert with no unique constraints`(testDB: TestDB) = runTest {
        /**
         * ```sql
         * CREATE TABLE IF NOT EXISTS tester (
         *      "name" VARCHAR(64) NOT NULL
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `upsert with manual update assignment`(testDB: TestDB) = runTest {
        withTables(testDB, Words) {
            val testWord = "Test"
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `upsert with manual update using insert values`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `upsert with update excluding columns`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `upsert with subQuery`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `batchUpsert with no conflict`(testDB: TestDB) = runTest {
        withTables(testDB, Words) {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `batchUpsert with conflict`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `batchUpdate with sequence`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `Upsert With UUID PrimaryKey`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `BatchUpsert With UUID PrimaryKey`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in TestDB.ALL_POSTGRES }
