...
class Ex05_Delete: R2dbcExposedTestBase() {
    ...
    fun `delete 01`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `delete with limit`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `delete with single join`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, userData ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `delete with multiple alias joins`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `delete with join query`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB !in TestDB.ALL_H2 }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `delete ignore with join`(testDB: TestDB) = runTest {
