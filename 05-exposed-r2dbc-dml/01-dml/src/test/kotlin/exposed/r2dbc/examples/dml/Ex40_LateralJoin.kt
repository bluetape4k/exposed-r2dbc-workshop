...
class Ex40_LateralJoin: R2dbcExposedTestBase() {
    ...
    fun `lateral join query`(testDB: TestDB) = runTest {
        ...
        withTestTablesAndDefaultData(testDB) { parent, child ->
            ...
            rows.forEach {
                log.debug {
                    "parent.id=${it[parent.id]}, parent.value=${it[parent.value]}, child.value=${it[subqueryAlias[child.value]]}"
                }
            }
        }
    }
    ...
    fun `lateral join query alias`(dialect: TestDB) = runTest {
    ...
    @ParameterizedTest
    @FieldSource("lateralJoinSupportedDb")
    fun `lateral direct table join`(dialect: TestDB) = runTest {
    ...
    private suspend fun withTestTables(testDB: TestDB, statement: suspend R2dbcTransaction.(Parent, Child) -> Unit) {
        withTables(testDB, Parent, Child) {
            statement(Parent, Child)
        }
    }

    private suspend fun withTestTablesAndDefaultData(
        testDB: TestDB,
        statement: suspend R2dbcTransaction.(Parent, Child) -> Unit,
    ) {
