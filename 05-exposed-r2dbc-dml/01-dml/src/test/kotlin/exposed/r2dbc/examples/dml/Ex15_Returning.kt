...
class Ex15_Returning: R2dbcExposedTestBase() {
    ...
    fun `insert returning`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `insert ignore returning`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in returningSupportedDb }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `upsert returing`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in returningSupportedDb }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `upsert returning with DAO`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in returningSupportedDb }
        ...
        withTables(testDB, Items) {
            ...
            result2[Items.id].value shouldBeEqualTo 1
            result2[Items.name] shouldBeEqualTo "B"
            result2[Items.price] shouldBeEqualTo 200.0
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `returning with no results`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in returningSupportedDb }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `delete returning`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in returningSupportedDb }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `update returning`(testDB: TestDB) = runTest {
