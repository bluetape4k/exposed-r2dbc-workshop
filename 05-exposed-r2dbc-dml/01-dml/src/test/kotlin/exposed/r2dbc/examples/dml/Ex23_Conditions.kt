...
class Ex23_Conditions: R2dbcExposedTestBase() {
    ...
    fun `TRUE and FALSE Ops`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `null safe equality Ops`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `comparison operations with EntityID columns`(testDB: TestDB) = runTest {
        val longTable = object: LongIdTable("long_table") {
            val amount = long("amount")
        }

        suspend fun selectIdWhere(condition: suspend SqlExpressionBuilder.() -> Op<Boolean>): List<Long> {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `same column used in slice multiple times`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, _, _ ->
            val row = cities
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `compare to nullable column`(testDB: TestDB) = runTest {
        val table = object: IntIdTable("foo") {
            val c1 = integer("c1")
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `nullOp update and select`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `nullOp update fails when apply to non-null column`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `nullOp in case`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, _, _ ->
            ...
            cities
                .select(cities.id, cities.name, caseCondition)
                .collect {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `CaseWhenElse as argument`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, _, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `chained and nested CaseWhenElse syntax`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, _, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `select aliased comparison result`(testDB: TestDB) = runTest {
        val table = object: IntIdTable("foo") {
            val c1 = integer("c1")
            val c2 = integer("c2").nullable()
        }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `null or empty`(testDB: TestDB) = runTest {
        /**
         * ```sql
