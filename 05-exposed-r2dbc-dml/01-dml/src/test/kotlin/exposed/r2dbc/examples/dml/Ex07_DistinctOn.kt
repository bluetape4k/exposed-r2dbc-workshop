...
class Ex07_DistinctOn: R2dbcExposedTestBase() {
    ...
    fun `distinctOn method`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `exception when distinct and distinctOn`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in distinctOnSupportedDb }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `empty distinctOn`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in distinctOnSupportedDb }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `distinctOn with count`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in distinctOnSupportedDb }
