...
class Ex08_Count: R2dbcExposedTestBase() {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `특정 컬럼의 count, countDistinct 함수 적용하기`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `count returns right value for Query with group by`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `count alias with table schema`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `count with offset and limit`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB !in TestDB.ALL_MYSQL_MARIADB }
