...
class Ex14_MergeTable: Ex14_MergeBase() {
    ...
    fun `mergeFrom with whenNotMatchedInsert`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `mergeFrom with whenNotMatchedInsert by alias`(testDB: TestDB) = runTest {
        withMergeTestTablesAndDefaultData(testDB) { dest, source ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `mergeFrom with whenMatchedUpdate`(testDB: TestDB) = runTest {
        withMergeTestTablesAndDefaultData(testDB) { dest, source ->
            dest.mergeFrom(
                source,
                on = { defaultOnCondition() }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `mergeFrom with whenMatchedUpdate by alias`(testDB: TestDB) = runTest {
        withMergeTestTablesAndDefaultData(testDB) { dest, source ->
            val destAlias = dest.alias("dest_alias")
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `mergeFrom with whenMatchedDelete`(testDB: TestDB) = runTest {
        withMergeTestTablesAndDefaultData(testDB) { dest, source ->
            dest.mergeFrom(
                source,
                on = { defaultOnCondition() }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `mergeFrom with whenNotMatchedInsert and whenMatchedUpdate`(testDB: TestDB) = runTest {
        withMergeTestTablesAndDefaultData(testDB) { dest, source ->
            dest.mergeFrom(
                source,
                on = { defaultOnCondition() }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `mergeFrom with whenMatchedDelete and condition`(testDB: TestDB) = runTest {
        withMergeTestTablesAndDefaultData(testDB) { dest, source ->
            dest.mergeFrom(
                source,
                on = { defaultOnCondition() }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `mergeFrom with multiple clauses`(testDB: TestDB) = runTest {
        withMergeTestTablesAndDefaultData(testDB) { dest, source ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `auto generated on condition`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `mergeFrom with whenNotMatched do nothing in postgres`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `mergeFrom with whenMatched do nothing in postgres`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue(testDB in TestDB.ALL_POSTGRES)
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `with overridingSystemValue option in postgres`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue(testDB in TestDB.ALL_POSTGRES)
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `with overridingUserValue option in postgres`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue(testDB in TestDB.ALL_POSTGRES)
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `insert default value`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue(testDB in TestDB.ALL_POSTGRES)
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `mergeFrom with const condition`(testDB: TestDB) = runTest {
