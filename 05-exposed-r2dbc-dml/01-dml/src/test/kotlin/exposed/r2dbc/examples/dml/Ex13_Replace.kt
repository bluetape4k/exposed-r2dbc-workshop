...
class Ex13_Replace: R2dbcExposedTestBase() {
    ...
    fun `replace select`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `replace select with specific columns`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in replaceSupported }
        ...
        withTables(testDB, NewAuth) {
            ...
            val expectedRowCount = if (testDB in TestDB.ALL_MYSQL_MARIADB_LIKE) 2 else 1
            affectedRowCount shouldBeEqualTo expectedRowCount
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `replace with PK conflict`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in replaceSupported }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `replace With Composite PK Conflict`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in replaceSupported }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `replace With Expression`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in replaceSupported }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `empty replace`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in replaceSupported }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `Batch Replace 01`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in replaceSupported }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `Batch Replace With Sequence`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in replaceSupported }
