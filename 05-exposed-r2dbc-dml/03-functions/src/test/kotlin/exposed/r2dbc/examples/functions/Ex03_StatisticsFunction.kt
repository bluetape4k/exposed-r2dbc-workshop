...
class Ex03_StatisticsFunction: Ex00_FunctionBase() {
    ...
    fun `StdDev Population`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `StdDev Sample`(testDB: TestDB) = runTest {
        withSampleTable(testDB) {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `Calc standard deviation`(testDB: TestDB) = runTest {
        withSampleTable(testDB) {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `VarPop function`(testDB: TestDB) = runTest {
        withSampleTable(testDB) {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `VarSamp function`(testDB: TestDB) = runTest {
        withSampleTable(testDB) {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `Calc variance`(testDB: TestDB) = runTest {
        withSampleTable(testDB) {
    ...
    private suspend fun withSampleTable(testDB: TestDB, body: suspend R2dbcTransaction.(TestDB) -> Unit) {
    ...
    private suspend infix fun org.jetbrains.exposed.v1.core.Function<BigDecimal?>.shouldExpressionEqualTo(expected: BigDecimal) {
    ...
    fun BigDecimal.simpleSqrt(): BigDecimal {
        if (this < BigDecimal.ZERO) {
            throw kotlin.ArithmeticException("Square root of negative number")
        }
        if (this == BigDecimal.ZERO) {
