...
class Ex01_Functions: Ex00_FunctionBase() {
    ...
    fun `calc function`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `sumLong function`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, _, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `calc function 02`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `calc function 03`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, users, userData ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `calc function 02 - Decimal`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, users, userData ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `rem on numeric PK should work`(testDB: TestDB) = runTest {
        /**
         * ```sql
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `mod on numeric PK should work`(testDB: TestDB) = runTest {
        /**
         * ```sql
         * CREATE TABLE IF NOT EXISTS test_mod_on_pk (
         *      id SERIAL PRIMARY KEY,
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `bitwiseAnd 01`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `bitwiseAnd 02`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `bitwiseOr 01`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
            val extra = 0b10
            val flagsWithExtra = users.flags bitwiseOr extra

            val rows = users.select(users.id, flagsWithExtra).orderBy(users.id).toFastList()
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `bitwiseOr 02`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
            val extra = 0b10
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `bitwiseXor 01`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `bitwiseXor 02`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `flag 01`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `substring 01`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `CharLength with Sum`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, _, _ ->
            val sumOfLength = CharLength(cities.name).sum()
            val expectedValue = cities.selectAll().toFastList().sumOf { it[cities.name].length }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `CharLength with edge case Strings`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `select case 01`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
            val field = case()
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `String functions`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, _, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `Locate functions 01`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, _, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `Locate functions 02`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, _, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `Random Function 01`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `regexp 01`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `regexp 02`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `concat 01`(testDB: TestDB) = runTest {
        withDb(testDB) {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `concat 02`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
            val concatField: Concat = concat(
                users.id,
                stringLiteral(" - "),
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `concat with numbers`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `custom string functions 01`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, _, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `custom string functions 02`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, _, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `custom integer function 01`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, _, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `custom integer function 02`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { cities, _, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `And operator doesn't mutate`(testDB: TestDB) = runTest {
        withDb(testDB) {
            val initialOp = DMLTestData.Cities.name eq "foo"
            val secondOp = DMLTestData.Cities.name.isNotNull()
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `Or operator doesn't mutate`(testDB: TestDB) = runTest {
        withDb(testDB) {
            val initialOp = DMLTestData.Cities.name eq "foo"
            val secondOp = DMLTestData.Cities.name.isNotNull()
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `And Or combinations`(testDB: TestDB) = runTest {
        withDb(testDB) {
            val initialOp = DMLTestData.Cities.name eq "foo"
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `custom operator`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `coalesce function`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `concat using plus operator`(testDB: TestDB) = runTest {
        withCitiesAndUsers(testDB) { _, users, _ ->
