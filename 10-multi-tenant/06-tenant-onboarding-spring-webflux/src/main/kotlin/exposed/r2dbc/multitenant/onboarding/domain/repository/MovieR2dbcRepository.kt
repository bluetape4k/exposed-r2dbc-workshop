package exposed.r2dbc.multitenant.onboarding.domain.repository

import exposed.r2dbc.multitenant.onboarding.domain.model.MovieActorCountRecord
import exposed.r2dbc.multitenant.onboarding.domain.model.MovieRecord
...
import exposed.r2dbc.multitenant.onboarding.domain.model.MovieWithProducingActorRecord
import exposed.r2dbc.multitenant.onboarding.domain.model.toActorRecord
import exposed.r2dbc.multitenant.onboarding.domain.model.toMovieRecord
import exposed.r2dbc.multitenant.onboarding.domain.model.toMovieWithActorRecord
import exposed.r2dbc.multitenant.onboarding.domain.model.toMovieWithProducingActorRecord
...
class MovieR2dbcRepository: R2dbcRepository<Long, MovieRecord> {
    ...
    suspend fun getMovieWithActors(movieId: Long): MovieWithActorRecord? {
        ...
        val pairs = MovieActorJoin
            ...
            .orderBy(ActorTable.id)
            .map { row ->
                row.toMovieRecord() to row.toActorRecord()
            }
            .toList()
        ...
        val movie = pairs.first().first
        val actors = pairs.map { it.second }
        return movie.toMovieWithActorRecord(actors)
    }
