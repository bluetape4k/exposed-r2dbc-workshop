...
```mermaid
flowchart LR
    Request[HTTP request] --> Auth[Spring Security authentication]
    Auth --> TenantAuth[AuthorizedTenantContextWebFilter]
    TenantAuth --> Reactor[Reactor tenant context]
    Reactor --> Exposed[Exposed R2DBC transaction]
    Exposed --> Router[TenantRoutingConnectionFactory]
    Router --> Korean[(korean DB)]
    Router --> English[(english DB)]
```
