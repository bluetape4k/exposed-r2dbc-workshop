...
import exposed.r2dbc.examples.json.JsonTestData.JsonArrayTable
import exposed.r2dbc.examples.json.JsonTestData.JsonTable
...
class Ex01_JsonColumn: R2dbcExposedJsonTest() {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `update with json`(testDB: TestDB) = runTest {
    ...
    private fun Column<DataHolder>.extract(string: String, toScalar: Boolean) {}
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `json contains`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `json contains with iterables`(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in jsonContainsSupported }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `logger with json collections`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `json with nullable column`(testDB: TestDB) = runTest {
        /**
         * ```sql
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `json with upsert`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `json with transformer`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `json as default`(testDB: TestDB) = runTest {
