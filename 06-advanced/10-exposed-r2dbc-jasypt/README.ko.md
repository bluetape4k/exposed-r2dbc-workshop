...
classDiagram
    ...
    note for JasyptVarCharColumn "결정적 암호화 → WHERE 절 검색 가능"
    note for JasyptBinaryColumn "결정적 암호화 → WHERE 절 검색 가능"

    IColumnType <|.. JasyptVarCharColumn
    IColumnType <|.. JasyptBinaryColumn
    JasyptVarCharColumn --> StringEncryptor : 위임
    JasyptBinaryColumn --> ByteEncryptor : 위임
