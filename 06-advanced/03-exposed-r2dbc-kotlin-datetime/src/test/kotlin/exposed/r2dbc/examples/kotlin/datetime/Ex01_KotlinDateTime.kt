...
class Ex01_KotlinDateTime: R2dbcExposedTestBase() {
    ...
    fun kotlinTimeFunctions(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun testStoringLocalDateTimeWithNanos(testDB: TestDB) = runTest {
        /**
         * ```sql
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `test selecting Instant using expressions`(testDB: TestDB) = runTest {
        /**
         * ```sql
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun testLocalDateComparison(testDB: TestDB) = runTest {
        /**
         * ```sql
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun testLocalDateTimeComparison(testDB: TestDB) = runTest {
        /**
         * ```sql
...
//                listOf(TestDB.ORACLE, TestDB.H2_V2_ORACLE, TestDB.H2_V2_PSQL, TestDB.H2_V2_SQLSERVER)
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun testTimestampWithTimeZone(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun testTimestampWithTimeZoneThrowsExceptionForUnsupportedDialects(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB in timestampWithTimeZoneUnsupportedDB }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun testTimestampWithTimeZoneExtensionFunctions(testDB: TestDB) = runTest {
        Assumptions.assumeTrue { testDB !in (timestampWithTimeZoneUnsupportedDB + TestDB.MYSQL_V8) }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun testCurrentDateTimeFunction(testDB: TestDB) = runTest {
//        Assumptions.assumeTrue { testDB !in TestDB.ALL_H2_V1 }

        withDb(testDB) {
            val now = Table.Dual.select(CurrentDateTime).single()[CurrentDateTime]
            log.debug { "now=$now" }
        }
    }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun testInfiniteDuration(testDB: TestDB) = runTest {
        /**
         * ```sql
         * CREATE TABLE IF NOT EXISTS tester (
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun testDateTimeAsArray(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun testSelectByTimeLiteralEquality(testDB: TestDB) = runTest {
        /**
         * ```sql
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun testCurrentDateAsDefaultExpression(testDB: TestDB) = runTest {
