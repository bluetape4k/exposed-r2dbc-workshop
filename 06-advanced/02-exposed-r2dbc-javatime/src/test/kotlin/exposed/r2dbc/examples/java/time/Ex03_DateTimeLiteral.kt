...
class Ex03_DateTimeLiteral: R2dbcExposedTestBase() {
    ...
    fun `select by date literal equality`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `select by Date Literal Comparison`(testDB: TestDB) = runTest {
        withTables(testDB, TableWithDate) {
            TableWithDate.insert {
                it[date] = defaultDate
            }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `select By DateTime Literal Equality`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `select By DateTime Literal Comparison`(testDB: TestDB) = runTest {
        withTables(testDB, TableWithDateTime) {
            TableWithDateTime.insert {
                it[dateTime] = defaultDateTime
            }
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `select By Timestamp Literal Equality`(testDB: TestDB) = runTest {
    ...
    @ParameterizedTest
    @MethodSource(ENABLE_DIALECTS_METHOD)
    fun `select By Timestamp Literal Comparison`(testDB: TestDB) = runTest {
        withTables(testDB, TableWithTimestamp) {
            TableWithTimestamp.insert {
                it[timestamp] = defaultTimestamp
            }
