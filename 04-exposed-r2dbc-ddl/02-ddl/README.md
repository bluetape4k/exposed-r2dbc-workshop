...
sequenceDiagram
    ...
    SU ->> DB: SHOW TABLES / query information_schema
    DB -->> SU: Current schema info
    SU ->> SU: Detect missing tables/columns
    SU ->> DB: ALTER TABLE ... ADD COLUMN ...
    DB -->> SU: Migration complete
    SU -->> TX: Missing items added
