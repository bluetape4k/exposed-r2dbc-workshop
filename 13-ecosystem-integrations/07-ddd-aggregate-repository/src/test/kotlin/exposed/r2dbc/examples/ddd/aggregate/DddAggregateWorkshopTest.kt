...
class DddAggregateWorkshopTest: AbstractR2dbcExposedTest() {

    private val database: R2dbcDatabase = R2dbcDatabase.connect(
        "r2dbc:pool:h2:mem:///ddd-aggregate-workshop;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=FALSE;MODE=PostgreSQL"
    )
    ...
    @Test
    fun `aggregate rejects invalid commands before Exposed tables are involved`() = runTest {
        val emptyLinesFailure = try {
            PlaceOrderCommand(
        ...
        emptyLinesFailure.shouldNotBeNull()

        val invalidQuantityFailure = try {
            OrderLine(
                sku = Sku("book"),
                quantity = 0,
        ...
        invalidQuantityFailure.shouldNotBeNull()
    }

    @Test
    fun `repository persists aggregate state and records the initial domain event`() = runTest {
    ...
    @Test
    fun `aggregate uses typed id and domain event metadata from bluetape core`() = runTest {
        val order = PurchaseOrder.place(newOrderCommand(), occurredAtEpochMillis = 1_000L)
    ...
    @Test
    fun `loaded aggregate appends approval event after the placed event`() = runTest {
        val placed = PurchaseOrder.place(newOrderCommand(), occurredAtEpochMillis = 1_000L)
        val placedSnapshot = repository.save(placed)

        val loaded = repository.findAggregate(OrderNumber("ORD-1000")).shouldNotBeNull()
        ...
        events.map { it.payload } shouldBeEqualTo listOf(
    ...
    @Test
    fun `repository rollback leaves aggregate rows and event rows uncommitted`() = runTest {
        val order = PurchaseOrder.place(newOrderCommand(), occurredAtEpochMillis = 1_000L)
    ...
    @Test
    fun `aggregate prevents an approval command from running twice`() = runTest {
        val placed = PurchaseOrder.place(newOrderCommand(), occurredAtEpochMillis = 1_000L)
