...
실행합니다. Checkpointable batch sibling은 공개된 안정 release
`io.github.bluetape4k.exposed:bluetape4k-exposed-batch:2.0.0` R2DBC reader,
writer, metadata repository를 직접 조합합니다. H2 test는 commit된 target ID를
중복하지 않고 `STOPPED`와 `FAILED` restart를 모두 증명하며,
`exposed-workshop`의 JDBC sibling과는 별도로 유지합니다. 안정 provider release와
catalog 좌표는 module README에 기록합니다.
