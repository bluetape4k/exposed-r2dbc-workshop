...
| 위험 | 조기 신호 | 완화·검증 | 재실행 |
|---|---|---|---|
| provider API drift | catalog가 artifact를 해석하지 못하거나 generic signature compile error | versionless catalog alias, resolved 2.0.0-SNAPSHOT API, compileKotlin/compileTestKotlin | `./gradlew :09-checkpointable-r2dbc-batch:compileKotlin :09-checkpointable-r2dbc-batch:compileTestKotlin --no-daemon --console=plain` |
...
| chunk/checkpoint drift | provider가 FAILED report checkpoint를 잃고 metadata를 null로 갱신 | 2.0.0-SNAPSHOT의 #747 보존 계약과 checkpoint `3`/restart `4..8` 회귀를 검증; target `sourceId`는 중복을 관측할 뿐 멱등성을 보장하지 않음 | stable provider promotion 때 version/import/test 재검토 |
...
slice를 재실행한다. 현재 개발 버전 provider 계약은 통과했으며 PR 전 live
CI/review 확인을 남긴다.
