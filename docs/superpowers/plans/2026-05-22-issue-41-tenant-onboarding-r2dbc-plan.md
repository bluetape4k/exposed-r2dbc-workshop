# Issue #41 Tenant Onboarding and Provisioning R2DBC 구현 계획
...
1. 커밋: spec/plan before implementation.
2. 생성: the new module by borrowing #05/#04 structure, then trim to the #41
   scope.
3. 구현: core value types, registry schema, dynamic routing, and
   provisioning in that order.
4. 추가: controller/API behavior and admin guard.
5. 추가: tests from lowest-level contracts to WebFlux integration.
6. 추가: README EN/KO and PNG diagram asset.
7. Wire Gradle/settings/Examples workflow and run verification.
8. 실행: 단계 6-R dual code review, fix P0/P1, create lesson, PR, PR review,
...
- Confirm worktree branch `feat/issue-41-tenant-onboarding` is based on current
  `origin/develop`.
- 유지: root `.codex/` untracked and out of commits.
- 커밋: spec/plan with Lore trailers 다음 위치 뒤: 단계 3-R closes.

### 1. Module Skeleton

- 추가: `10-multi-tenant/06-tenant-onboarding-spring-webflux`.
...
- 유지: package root `exposed.r2dbc.multitenant.onboarding`.

### 2. Core Tenant Types and Validation

- 추가: `TenantId` value class with regex validation through bluetape4k
  validation helpers where practical.
- 추가: unit tests for valid slug and invalid blank, uppercase, slash, dot,
  colon, and overlength values.
- 추가: request/response DTOs:
...
- 추가: startup initializer/recovery:
...
- 구현: `TenantConnectionFactoryRegistry`:
...
- 구현: `TenantRoutingConnectionFactory` directly as `ConnectionFactory`:
...
- 추가: `ProvisioningFailureSimulator`:
...
- 구현: admin guard for `POST /api/tenants`:
  - header `X-ADMIN-TOKEN`.
  - property `app.onboarding.admin-token`.
  - constant-time byte comparison.
  - missing/wrong -> `401`.
- 구현: `TenantOnboardingController`:
  - `POST /api/tenants`.
  - status mapping: `201`, `400`, `401`, `409`, `429`, `500`.
- 구현: actor endpoints from #04/#05:
  - use `X-TENANT-ID` only for routing.
  - do not treat `X-ADMIN-TOKEN` as tenant authorization.
  - unknown tenant -> `404`.
- 추가: exception handler/problem responses without raw URLs or stack traces.
...
- `TenantConnectionFactoryRegistryTest`:
  ...
  - destroy closes every pool and does not skip later pools 다음 위치 뒤: one close
    failure.
...
- `TenantProvisionerTest`:
  ...
  - registry outage at reservation and 다음 위치 뒤: reservation maps to
    `TenantProvisioningException`.
  ...
  - retry 다음 위치 뒤: runtime failure succeeds.
...
- `ArchitectureTest`:
  ...
  - `TenantProvisioner` and routing infrastructure accept `TenantId` rather
    than raw `String` 다음 위치 뒤: controller boundary.

사용: `runSuspendIO` for R2DBC IO and bluetape4k assertions for new tests.
...
- 추가: module `README.md` and `README.ko.md`:
  ...
  - H2 in-memory restart semantics: stale `ACTIVE` rows are marked `FAILED`;
    users should re-onboard tenants 다음 위치 뒤: restart.
...
- 추가: English KDoc for public classes/value types introduced by the module.
...
- 금지: expand Nightly in this issue; record #42 deferral in README and lesson.
...
|---|---|---|---|
| P1 | Registry reserve must include the spec-required per-tenant mutex, not only a transaction. | Accepted | 추가함: per-tenant `Mutex` to `TenantRegistryRepository.reserve()` task. |
| P1 | Startup recovery must complete before WebFlux accepts requests. | Accepted | 추가함: `SmartLifecycle` or equivalent startup gate task. |
| P1 | Same-tenant concurrent reserve test was missing. | Accepted | 추가함: N-coroutine duplicate reserve test. |
| P2 | Custom routing factory deviation, URL leak assertions, registry-outage routing, suspend cleanup, and post-controller `TenantId` contract needed explicit tasks. | Accepted | 추가함: plan tasks/tests for each item. |
...
### Integrated 단계 3-R Findings

| Priority | Count | Status |
|---|---:|---|
| P0 | 0 | None in initial advisor review. |
| P1 | 0 | Closed 다음 위치 뒤: Claude rerun `.omx/artifacts/claude-plan-issue-41-tenant-onboarding-rerun-20260522214344.md`. |
| P2 | 0 | Actionable P2s applied before implementation. |
| P3 | 0 | 차단성 nit은 남기지 않았다. |

### 11. Review, Lessons, PR, Merge

- 단계 6-R dual code review:
...
- 푸시: branch and open PR in English.
- 단계 7-R:
...
| Lessons/PR/merge/sync covered | Done | 단계 11 covers full requested sequence. |
