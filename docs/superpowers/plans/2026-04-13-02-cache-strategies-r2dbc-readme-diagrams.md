# 02-cache-strategies-r2dbc README Diagram Refresh Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** `11-high-performance/02-cache-strategies-r2dbc/README.md`의 Mermaid 다이어그램을 개선해 캐시 조회 흐름과 CacheRepository 구조를 더 읽기 쉽게 만들고, 보조 architecture diagram을 추가한다.

**Architecture:** README는 GitHub 렌더링 호환성을 위해 Mermaid만 사용한다. 다이어그램은 `architecture → classDiagram → cache flow` 순으로 재배치하고, classDiagram은 전략/책임 중심으로 축약하며, flowchart는 L1/L2/DB 경로와 HIT/MISS/FILL을 색상과 subgraph로 명확히 구분한다.

**Tech Stack:** Markdown, Mermaid (`flowchart`, `classDiagram`), GitHub README rendering
...
- Reference: `docs/superpowers/specs/2026-04-13-02-cache-strategies-r2dbc-readme-diagrams-design.md`
- Optional verification note: `docs/testlogs/2026-04.md` (문서 전용 변경이면 기록 생략 가능)

## Task 1: README 다이어그램 배치 재구성

**Files:**
- Modify: `11-high-performance/02-cache-strategies-r2dbc/README.md`
- Reference: `docs/superpowers/specs/2026-04-13-02-cache-strategies-r2dbc-readme-diagrams-design.md`

- [ ] **Step 1: 현재 README의 대상 구간을 확인한다**
...
- [ ] **Step 2: architecture diagram 섹션 제목과 설명 문장을 추가한다**
...
- [ ] **Step 3: architecture diagram Mermaid 블록을 추가한다**
...
- [ ] **Step 4: README에서 architecture → class → flow 순서가 되도록 섹션 위치를 정리한다**
...
- [ ] **Step 5: 변경된 Markdown 블록 문법을 눈으로 검토한다**
...
## Task 2: CacheRepository classDiagram 개선

**Files:**
- Modify: `11-high-performance/02-cache-strategies-r2dbc/README.md`

- [ ] **Step 1: 기존 classDiagram 제목을 더 직접적인 이름으로 바꾼다**
...
- [ ] **Step 2: 기존 classDiagram 블록을 전략/책임 중심 버전으로 교체한다**
...
- [ ] **Step 3: classDiagram 아래 짧은 설명 문장을 추가한다**
...
- [ ] **Step 4: 다이어그램 텍스트가 너무 길면 1회 축약한다**
...
- [ ] **Step 5: Mermaid classDiagram 문법을 재검토한다**
...
## Task 3: 캐시 조회 흐름 flowchart 개선

**Files:**
- Modify: `11-high-performance/02-cache-strategies-r2dbc/README.md`

- [ ] **Step 1: 기존 flowchart를 유지하되 제목은 그대로 둔다**
...
- [ ] **Step 2: flowchart를 계층/분기 강조 버전으로 교체한다**
...
- [ ] **Step 3: flowchart 아래 설명 문장을 보강한다**
...
- [ ] **Step 4: 빠른 경로와 느린 경로가 시각적으로 구분되는지 검토한다**
...
**Files:**
- Modify: `11-high-performance/02-cache-strategies-r2dbc/README.md`
- Optional verification note: `docs/testlogs/2026-04.md`

- [ ] **Step 1: README 전체를 다시 읽고 다이어그램 순서와 중복 설명을 확인한다**
...
- [ ] **Step 2: Mermaid 문법 오류 가능성을 집중 확인한다**
...
- [ ] **Step 3: 문서 전용 변경인지 판단하고 검증 범위를 기록한다**

판단 기준:

```text
If only README.md changed:
- 코드/테스트 미실행 가능
- testlog 기록 생략 가능
- 최종 보고에서 문서 전용 변경임을 명시
```

- [ ] **Step 4: git diff로 README 변경 범위가 요청 범위를 넘지 않았는지 확인한다**
...
- [ ] **Step 5: 완료 보고에 포함할 핵심 포인트를 정리한다**
...
- CacheRepository class diagram 개선: Task 2
- 캐시 조회 흐름 개선: Task 3
- 최소 설명 보강 및 문법 검토: Task 1~4
