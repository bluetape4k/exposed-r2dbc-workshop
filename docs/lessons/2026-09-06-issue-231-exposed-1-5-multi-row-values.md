...
- `multi row insert ignore records actual table count`: PostgreSQL/MariaDB의 부분 충돌에서 client-side 반환 행과 실제 테이블 행 수를 분리해 확인한다.
- 기존 `batch insert number of inserted rows`: 충돌을 무시한 실제 삽입 수를 검증하는 별도 custom executable 예제다.

## 검증 드라이버

테스트 런타임에서 실제로 해석된 R2DBC driver 버전은 다음과 같다. 버전은
`gradle/libs.versions.toml`과 각 Testcontainers 실행 classpath에서 확인했다.

| 데이터베이스 | R2DBC driver | 버전 |
|---|---|---|
| H2 | `io.r2dbc:r2dbc-h2` | `1.1.0.RELEASE` |
| PostgreSQL | `org.postgresql:r2dbc-postgresql` | `1.1.1.RELEASE` |
| MariaDB | `org.mariadb:r2dbc-mariadb` | `1.4.0` |
| MySQL 8 | `io.asyncer:r2dbc-mysql` | `1.4.1` |

## 제한 사항

`ignore = true`인 multi-row 부분 충돌은 driver의 per-entry update count 제공 여부에 의존한다. H2와 MariaDB는 이 count를 제공하지 않으므로 반환 행을 항상 실제 삽입 행과 동일하다고 해석하면 안 된다. 따라서 공용 seed는 기존처럼 `shouldReturnGeneratedValues = false`를 유지하고, PostgreSQL/MariaDB Testcontainers에서 실제 테이블 행 수를 별도로 확인한다.
